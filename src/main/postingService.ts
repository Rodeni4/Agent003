...
async function fillPostForm(page: Page, text: string, imagePath?: string) {
    const editor = page
...
async function publishToOwnWall(page: Page, text: string, imagePath?: string) {
    ...
    await fillPostForm(page, text, imagePath);
}

async function publishToOkGroup(page: Page, groupId: string, text: string, imagePath?: string) {
...
export async function publishOkTextPost(
    ...
    try {
        if (payload.publishToWall) {
            await publishToOwnWall(page, text, payload.imagePath);
            publishedTargets.push('стена OK');
        }

        if (payload.publishToGroup && groupId) {
            await publishToOkGroup(page, groupId, text, payload.imagePath);
